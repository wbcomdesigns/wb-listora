...
| D.listing-video-renders | #10194472456 | The listing `video` field accepted and stored a URL since it shipped - submission input, REST save, meta schema, field registry, and the edit form read it back - with NO render path anywhere. An owner could add a YouTube URL, see it persist in `GET /listings/{id}/detail`, and find it on exactly zero surfaces | Set `meta.video` to a YouTube URL on a listing. Assert the detail page renders `.listora-detail__video` containing a real provider `<iframe>` (oEmbed, not a hand-rolled iframe - it respects the site's oEmbed settings and privacy filters). Then assert graceful degradation, all three: empty value, a garbage non-URL string, and a valid URL from a provider oEmbed cannot resolve - each must render NO section and NO iframe. A blank bordered section is worse than no section. Check the embed is fluid at 390px (aspect-ratio box, not the provider's fixed width). |

...
- **1.8.0: grid toolbar counter.** `listing-grid/toolbar.php` now binds its "Showing X-Y of Z" spans to `state.gridCountFrom`, `state.gridCountTo` and `state.gridCountTotal` instead of `state.pageFrom` / `pageTo` / `totalResults`. The new names give each grid its own count when a page has two or more grids. An override that still binds the old keys keeps working on single-grid pages, but on multi-grid pages every toolbar will show the last grid's range - update it to the new names.
- **1.8.0: per-review criteria stars.** Free prints each review's per-criterion stars on `wb_listora_review_after_content` (priority 5) with `wb_listora_render_review_criteria()`, so overridden `review-card.php` / `tabs.php` files get them as long as they keep firing that action. To hide them: `remove_action( 'wb_listora_review_after_content', 'wb_listora_render_review_criteria', 5 );`. To render them yourself, `wb_listora_get_review_criteria_scores( $review )` returns `[ [ 'label' => 'Food', 'rating' => 5 ], ... ]` for the criteria the listing type still configures.

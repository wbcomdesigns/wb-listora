# Social Snippets - Twitter/X + LinkedIn

Pre-written posts for launch, weekly cadence, and feature highlights. Mix and match. Each backed by an existing screenshot.

---

## Launch posts (use first 7 days)

### Twitter/X - Launch announcement

> WB Listora is the WordPress directory plugin that gives you the public directory FREE and the business model layer in Pro.
>
> Search. Reviews. Claims. Frontend submission. Anti-spam. Maps. Schema. Hooks. CLI. All free.
>
> Credit plans + lead capture + needs marketplace + moderators + analytics → Pro.
>
> wbcomdesigns.com

### LinkedIn - Launch announcement

> A directory plugin should do three things: let people find businesses, let businesses be found, let you make money doing it.
>
> Most WordPress directory plugins do the first two badly and leave the third as homework.
>
> WB Listora was built to do all three. Free covers the public site - 11 native Gutenberg blocks, denormalized search, geo + facet + keyword search, reviews, claims, frontend submission, 6 anti-spam layers.
>
> Pro adds the business model - credit plans with Hold-and-Commit activation, paid featured rotation, multi-criteria photo reviews, verification badges, moderator team, side-by-side comparison, BuddyPress activity sync, outgoing + inbound webhooks bridging Stripe / PayPal / WooCommerce / MemberPress / PMPro - and the reverse Needs Marketplace where buyers post requests and businesses respond.
>
> Free at wbcomdesigns.com. Pro at wbcomdesigns.com.

---

## Weekly cadence (1 per week, rotate themes)

### Theme: Speed

> Most WordPress directory plugins take a weekend to set up.
>
> WB Listora's setup wizard gets you from install to a working directory with demo content in 30 minutes.
>
> Then you customize at your pace, not the plugin's.
>
> Free at wbcomdesigns.com → [link]

### Theme: Scale

> Built our directory plugin's search on a denormalized index because `WP_Query` falls over at scale.
>
> 100K+ listings, full faceted filtering, geo radius queries, "Search this area" map-bounds support.
>
> No "upgrade your hosting" surprises.
>
> wbcomdesigns.com

### Theme: Vendor UX

> Vendors should never have to see wp-admin.
>
> WB Listora's `/my-listings/` dashboard:
> - Edit listings inline
> - Reply to reviews
> - Renew with one click
> - Manage services
> - See incoming leads (Pro)
> - Saved searches with alerts (Pro)
> - Needs marketplace responses (Pro)
>
> One frontend page. Zero wp-admin friction.

### Theme: Business model

> The reason most WordPress directory plugins don't make money for site owners:
>
> No clean way to charge vendors.
>
> WB Listora Pro fixes that. Credit packs + pricing plans + 7 payment integrations + coupons + Hold-and-Commit activation. Vendors pay, get credits, activate plans. You collect.
>
> wbcomdesigns.com

### Theme: Reverse marketplace

> Every directory plugin: vendors list, customers search.
>
> WB Listora Pro adds the Needs Marketplace: buyers post what they want, businesses respond with quotes.
>
> Two flywheels in one plugin. Both flywheels feed each other.
>
> wbcomdesigns.com

---

## Feature highlights (use as part of a weekly drip)

### Search & Filters

> Faceted search that updates without a page reload.
> Geo Near-Me with browser geolocation.
> URL state preserved so visitors can share or bookmark exact filter sets.
> All in Free.
> wbcomdesigns.com

📷 `search-and-filters.png`

### Multi-criteria Reviews

> One overall rating tells you nothing.
>
> WB Listora Pro lets reviewers rate per aspect - Food, Service, Value, Ambiance - each separately. Plus photo reviews. Plus owner replies. Plus helpful-vote milestones.
>
> Reviews that actually inform.

📷 `multi-criteria-reviews.png`

### Compare Listings

> Visitors compare. Make it easy.
>
> WB Listora Pro: heart-save up to 4 listings, hit Compare, see them side-by-side. Floating bar shows current selections across the whole site. Selections persist via localStorage.
>
> Conversion-tested UX from Amazon and Booking, on your directory.

📷 `compare-listings.png`

### Needs Marketplace

> Flip the directory.
>
> Instead of vendors waiting to be found, buyers post what they need: "caterer for 200 guests in Brooklyn." Matching businesses respond with quotes.
>
> WB Listora Pro's Needs Marketplace is a second flywheel in the same plugin.

📷 `needs-marketplace.png`

### Moderators

> You don't need to do every approval yourself.
>
> WB Listora Pro's Moderators feature: grant team members exactly the scopes they need - approve listings, approve claims, moderate reviews, resolve reports - and nothing else. Cap-gated at REST + admin.
>
> Safe delegation. Audit trail included.

📷 `moderators.png`, `moderation-queue.png`

### Analytics

> Listings, views, clicks, contact fills, lead volumes, top categories, top vendors.
>
> WB Listora Pro tracks every customer-visible action and shows you what's working.
>
> Plus an Audit Log for who-did-what.
>
> If you can't measure it, you can't grow it.

📷 `analytics.png`

### Verification Badges

> Trust signals matter. Verified badges differentiate vetted vendors from the long tail.
>
> WB Listora Pro's Verification Badges: define your badge types - Verified Owner, Top Rated, Editor's Pick - assign to listings, badges render on cards + detail + search facets. Each is configurable: color, icon, criteria, expiration.

📷 `verification-badges.png`

### Anti-spam

> WordPress directory plugins die from review spam.
>
> WB Listora's anti-spam stack - all on by default in Free:
>
> 1. Honeypot
> 2. Per-IP rate limits
> 3. reCAPTCHA v3 OR Cloudflare Turnstile
> 4. Akismet
> 5. Keyword blacklist
> 6. URL density cap
>
> Six layers. Spam has to defeat all six.

---

## Hashtag set (use sparingly - 2-3 per post)

`#WordPress` `#WordPressPlugin` `#WPDev` `#WPCommunity` `#DirectoryPlugin` `#WordPressDirectory` `#BusinessDirectory` `#LocalDirectory` `#SaaS` `#NoCode` `#WebDev` `#Gutenberg` `#WordPressBusiness`

For LinkedIn:
`#WordPress` `#WebDevelopment` `#DirectoryPlatform` `#SmallBusiness` `#MarketplaceTech` `#SaaSProducts`

## Response patterns (for replies + DMs)

| When someone asks… | Default reply |
|---|---|
| "How is this different from Directorist?" | "Block-first architecture (no shortcodes), denormalized search for 100K+ scale, and Pro includes a Needs Marketplace + Hold-and-Commit credit system. Side-by-side comparison page: https://wbcomdesigns.com/downloads/listora/" |
| "Is it free?" | "Yes - full public directory in Free at wbcomdesigns.com. Pro adds the business model layer (plans, lead forms, verification, etc.) at wbcomdesigns.com." |
| "Does it work with X theme?" | "Yes - it's a block-first plugin so any modern block theme works. We test against BuddyX Pro, Astra, Kadence, GeneratePress. Classic themes work too via template overrides." |
| "Can I migrate from [competitor]?" | "Yes - built-in migrators for Directorist, GeoDirectory, WPBDP, ListingPro. CLI + admin. Run `wp listora migrate --from=X --dry-run` to preview." |
| "Is there a demo?" | "Yes - wbcomdesigns.com has a live model directory with all features active. You can also load 9 demo packs locally via `wp listora demo seed --pack=all`." |

## Posting cadence guide

| Channel | Cadence | Best windows (US Eastern) |
|---|---|---|
| Twitter/X | 5-7 posts/week | 9-11am, 1-3pm |
| LinkedIn | 2-3 posts/week | 8-10am Tue-Thu |
| Reddit (r/WordPress, r/WPdev) | 1 post/month + organic replies | Weekday evenings |
| Facebook (WP Help & Share) | 1 post/week | Weekday afternoons |

## Related

- [Hero Pitch](hero-pitch.md) - voice + tone reference.
- [Video Scripts](video-script-features.md) - convert any 60s script into a 280-char tweet.
- [Landing Headlines](landing-headlines.md) - match social hooks to the variant currently being A/B-tested.

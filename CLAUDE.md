...
| **Still open** | LST-F-21 — `featured_only`, `verified_only` and `author` are read by the engine but never declared by REST, so they are accepted and ignored. Left unfixed deliberately: declaring them is additive public API surface and `author` is a product decision. |

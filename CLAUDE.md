...
| Journeys (executable) | [`tests/qa/journeys/`](tests/qa/journeys/) | 80 self-contained markdown flows an agent runs end-to-end via Playwright + WP-CLI + curl + mysql_query (17 customer / 16 admin / 45 regression / 2 system). Returns PASS/FAIL with exact step + likely_files for triage. See [`tests/qa/journeys/README.md`](tests/qa/journeys/README.md) for the schema. | Bug-fix + feature PRs (write); `bin/run-journeys.sh` (execute) |
...
Authored journeys (80 total — 17 customer / 16 admin / 45 regression / 2 system). The tables below are a curated highlight subset; `tests/qa/journeys/` is the full index:

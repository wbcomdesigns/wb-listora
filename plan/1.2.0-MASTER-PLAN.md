...
- Free run: resumeFromRunId wf_47ed0e77-0e0, manifest plan/1.2.0-plan-flowclosure.yaml (validated JSON regenerate via validate-plan.mjs). Wave-1 owner fixes: b1c2d65 + c9ddd91.
- Pro run: resumeFromRunId wf_0f0764b7-9d1, manifest ../wb-listora-pro/plan/1.2.0-plan-flowclosure.yaml. Wave-1 owner fix: 8bf6a11.
- Resume mechanics: pass {dry_run:false, manifest:<validated JSON>} with the resumeFromRunId; journey nudge notes already added so functional verifies re-run while implementer caches replay.
